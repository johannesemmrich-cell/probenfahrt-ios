...
enum PDFReportRenderer {
    ...
    static func renderSamplesReport(title: String, lines: [SamplesReportGenerator.ReportLine]) -> Data {
        let pageRect = CGRect(x: 0, y: 0, width: 595.2, height: 841.8) // A4 at 72dpi
        let renderer = UIGraphicsPDFRenderer(bounds: pageRect)
        ...
        return renderer.pdfData { context in
            ...
            if lines.isEmpty {
                let text = "Keine Proben-Meldungen in diesem Monat." as NSString
                text.draw(at: CGPoint(x: 40, y: y), withAttributes: bodyAttributes)
            } else {
                for line in lines {
                    let unit = line.daysWithSamples == 1 ? "Tag" : "Tage"
                    let text = "\(line.locationName): \(line.daysWithSamples) \(unit) mit Proben" as NSString
                    text.draw(at: CGPoint(x: 40, y: y), withAttributes: bodyAttributes)
                    y += 24
                }
            }
